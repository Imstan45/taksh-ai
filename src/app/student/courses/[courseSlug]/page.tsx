...
import { serviceNowProgram } from "@/lib/content-factory/servicenow-curriculum";
...
export default async function CoursePage({ params }: { params: Promise<{ courseSlug: string }> }) {
  ...
  return (
    <DashboardShell {...session.user}>
      ...
      {course.course.startsWith("ServiceNow")&&<section className="servicenow-course-banner"><div><small>Career-ready pathway</small><h3>{serviceNowProgram.promise}</h3><p>{serviceNowProgram.schedule} · {serviceNowProgram.learningMix} · {serviceNowProgram.projects.length} portfolio projects</p></div><div>{serviceNowProgram.tracks.map((track,index)=><span key={track}><b>0{index+1}</b>{track}</span>)}</div></section>}
