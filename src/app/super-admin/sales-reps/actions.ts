...
export async function changeSalesRepStatus(formData:FormData){const admin=await requireSuper(),id=String(formData.get("id")??""),status=String(formData.get("status")??"");if(!["active","inactive","suspended"].includes(status))throw new Error("Invalid status.");await prisma.$transaction(async tx=>{const rep=(await tx.$queryRaw<Array<{user_id:string}>>`update public.sales_reps set status=${status},updated_at=now() where id=${id}::uuid returning user_id`)[0];if(!rep)throw new Error("Sales Rep not found.");await tx.$executeRaw`update public.user_roles set account_status=${status==='active'?'active':'suspended'},authorization_version=authorization_version+1,updated_at=now() where user_id=${rep.user_id}::uuid`;await tx.$executeRaw`insert into public.audit_logs(actor_id,action,target_type,target_id,new_values) values(${admin.id}::uuid,'sales_rep.status_changed','sales_rep',${id},${JSON.stringify({status})}::jsonb)`});refresh()}

export async function promoteSelectedParticipant(formData:FormData){const admin=await requireSuper(),participantId=String(formData.get("participantId")??"");await prisma.$transaction(async tx=>{const row=(await tx.$queryRaw<Array<{user_id:string;display_name:string;email:string;referral_code:string}>>`select p.user_id,p.display_name,u.email,p.referral_code from public.sales_challenge_participants p join auth.users u on u.id=p.user_id where p.id=${participantId}::uuid and p.selection_status='selected' for update`)[0];if(!row)throw new Error("Only a selected participant can be promoted.");const rep=(await tx.$queryRaw<Array<{id:string}>>`insert into public.sales_reps(user_id,source_participant_id,full_name,email,referral_code,status,joined_at,approved_by) values(${row.user_id}::uuid,${participantId}::uuid,${row.display_name},${row.email},replace(${row.referral_code},'TSC-',''),'active',now(),${admin.id}::uuid) on conflict(user_id) do update set status='active',joined_at=coalesce(sales_reps.joined_at,now()),approved_by=excluded.approved_by,updated_at=now() returning id`)[0];await tx.$executeRaw`update public.user_roles set role='SALES_REP',institution_id=null,account_status='active',authorization_version=authorization_version+1,updated_at=now() where user_id=${row.user_id}::uuid`;await tx.$executeRaw`update public.sales_referral_attributions set sales_rep_id=${rep.id}::uuid where participant_id=${participantId}::uuid`;});refresh()}
