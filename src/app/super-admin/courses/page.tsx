...
import { courseAudience } from "@/lib/institution-content";
...
export default async function CourseOperationsPage() {
  ...
  const [courses, institutions, students, assignments] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ id: string; email: string; institution_type: string }>>`
      SELECT u.id, u.email, institution.institution_type FROM auth.users u JOIN public.user_roles r ON r.user_id = u.id
      JOIN public.institutions institution ON institution.id=r.institution_id
    ...
    <DashboardShell {...session.user}>
      <div className="grid gap-6 lg:grid-cols-2">
        <ActionFeedbackForm action={grantInstitutionCourse} successMessage="Course access granted successfully." pendingMessage="Granting course access…" className="glass-card space-y-4">
          ...
          <select className="field" name="course" required><option value="">Choose published course</option>{courses.map((item) => <option value={item.course} key={item.course}>{item.course} ({courseAudience(item.course)})</option>)}</select>
        ...
        <ActionFeedbackForm action={assignStudentCourse} successMessage="Course assigned to the student successfully." pendingMessage="Assigning course…" className="glass-card space-y-4">
          ...
          <select className="field" name="studentId" required><option value="">Choose student</option>{students.map((item) => <option value={item.id} key={item.id}>{item.email} ({item.institution_type})</option>)}</select>
          <select className="field" name="course" required><option value="">Choose course</option>{courses.map((item) => <option value={item.course} key={item.course}>{item.course} ({courseAudience(item.course)})</option>)}</select>
