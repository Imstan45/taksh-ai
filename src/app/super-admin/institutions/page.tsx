...
export default async function InstitutionsPage() {
  ...
  const institutions = await prisma.$queryRaw<Array<{ id: string; name: string; slug: string; active: boolean }>>`
    SELECT id, name, slug, active FROM public.institutions ORDER BY name
  `;
  return (
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        ...
        <section className="glass-card">
          ...
          <div className="mt-5 divide-y divide-white/10">
            {institutions.map((item) => <div className="flex items-center justify-between py-4" key={item.id}><div><b>{item.name}</b><p className="text-sm text-zinc-500">{item.slug}</p></div><span className="text-xs text-emerald-300">{item.active ? "Active" : "Inactive"}</span></div>)}
