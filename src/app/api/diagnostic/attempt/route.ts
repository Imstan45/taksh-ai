...
export async function POST(request:Request){
  ...
  const extremeReady=verification?false:(await prisma.$queryRaw<Array<{ready:boolean}>>`select count(*) filter(where category='quantitative_aptitude')>=3 and count(*) filter(where category='logical_reasoning')>=2 and count(*) filter(where category='english_verbal')>=2 and count(*) filter(where category='database_technical' and technical_track in(${parsed.data.technicalTrack},'general-it'))>=3 ready from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='extreme'`)[0]?.ready;
  const selected=verification?await prisma.$queryRaw<Array<{id:string}>>`
    ...
    (select id from public.diagnostic_questions where active and approved and pool_type='verification' and category='database_technical' and technical_track in (${parsed.data.technicalTrack},'general-it') order by (technical_track=${parsed.data.technicalTrack}) desc,md5(id||${salt}) limit 3)`:extremeReady?await prisma.$queryRaw<Array<{id:string}>>`
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='quantitative_aptitude' order by md5(id||${salt}) limit 7) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='logical_reasoning' order by md5(id||${salt}) limit 6) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='english_verbal' order by md5(id||${salt}) limit 6) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='database_technical' and technical_track in (${parsed.data.technicalTrack},'general-it') order by (technical_track=${parsed.data.technicalTrack}) desc,md5(id||${salt}) limit 11) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='extreme' and category='quantitative_aptitude' order by md5(id||${salt}) limit 3) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='extreme' and category='logical_reasoning' order by md5(id||${salt}) limit 2) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='extreme' and category='english_verbal' order by md5(id||${salt}) limit 2) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='extreme' and category='database_technical' and technical_track in (${parsed.data.technicalTrack},'general-it') order by (technical_track=${parsed.data.technicalTrack}) desc,md5(id||${salt}) limit 3)`:await prisma.$queryRaw<Array<{id:string}>>`
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='quantitative_aptitude' order by md5(id||${salt}) limit 10) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='logical_reasoning' order by md5(id||${salt}) limit 8) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='english_verbal' order by md5(id||${salt}) limit 8) union all
    (select id from public.diagnostic_questions where active and approved and pool_type='primary' and difficulty_label='standard' and category='database_technical' and technical_track in (${parsed.data.technicalTrack},'general-it') order by (technical_track=${parsed.data.technicalTrack}) desc,md5(id||${salt}) limit 14)`;
  ...
  const ids=shuffled(selected.map(item=>item.id)),orders=Object.fromEntries(ids.map(id=>[id,shuffledOptionKeys()]));
