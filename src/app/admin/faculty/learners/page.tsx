...
export default async function LearnersPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <DashboardShell {...session.user}><div><h2 className="text-3xl font-semibold">Students</h2><p className="mt-2 text-zinc-400">Open a student to review progress, assessments and feedback.</p></div><form className="mt-6 flex gap-3"><input className="field" name="q" defaultValue={q} placeholder="Search students by email"/><button className="btn-ghost">Search</button></form><section className="glass-card mt-6"><div className="space-y-3">{learners.map((learner)=><Link className="grid gap-3 rounded-xl border border-white/10 p-4 transition hover:border-violet-400/30 hover:bg-violet-500/5 md:grid-cols-5" href={`/admin/faculty/learners/${learner.id}`} key={learner.id}><b>{learner.email}</b><span>{learner.batch_name??"No batch"}</span><span>{learner.completion??0}% complete</span><span>{Number(learner.xp)} XP · {learner.streak??0} day streak</span><span>{learner.last_activity?learner.last_activity.toLocaleDateString():"No activity"}</span></Link>)}{!learners.length&&<div className="py-12 text-center text-sm text-zinc-500">No assigned students match this search.</div>}</div></section></DashboardShell>;
}
