...
export default async function FacultyDashboard() {
  ...
  return <DashboardShell {...session.user}><div className="grid gap-4 md:grid-cols-5">{[["Assigned batches",metrics.batches],["Assigned students",metrics.learners],["Available courses",metrics.courses],["Average completion",`${metrics.average_completion??0}%`],["Falling behind",metrics.falling_behind]].map(([label,value])=><section className="glass-card" key={label}><p className="text-sm text-zinc-400">{label}</p><p className="mt-3 text-3xl font-semibold">{String(value)}</p></section>)}</div><div className="mt-6 grid gap-4 md:grid-cols-2"><Link className="glass-card" href="/admin/faculty/learners"><h2 className="text-xl font-semibold">Students</h2><p className="mt-2 text-zinc-400">Open student profiles, review learning activity and provide feedback.</p></Link><Link className="glass-card" href="/admin/faculty/content"><h2 className="text-xl font-semibold">Teaching content</h2><p className="mt-2 text-zinc-400">View published lessons available to your institution.</p></Link></div></DashboardShell>;
}
