...
export default async function PeoplePage({ searchParams }: { searchParams: Promise<{ q?: string; role?: string }> }) {
  ...
  return <DashboardShell {...session.user}>
    ...
    <section className="glass-card mt-6"><h2 className="text-xl font-semibold">Institution people</h2><div className="mt-5 space-y-3">{people.map((person)=><ActionFeedbackForm action={updateInstitutionMember} successMessage={`${person.email} updated successfully.`} pendingMessage="Updating member…" className="grid gap-3 rounded-xl border border-white/10 p-4 md:grid-cols-[1fr_140px_1fr_1fr_auto]" key={person.user_id}><input type="hidden" name="userId" value={person.user_id}/><div><b>{person.email}</b><p className="text-xs text-zinc-500">{person.role} · {person.account_status}</p></div><select className="field" name="status" defaultValue={person.account_status}><option value="active">Active</option><option value="suspended">Suspended</option></select><select className="field" name="departmentId" defaultValue={person.department_id??""}><option value="">No department</option>{departments.map((item)=><option value={item.id} key={item.id}>{item.name}</option>)}</select><select className="field" name="batchId" defaultValue={person.batch_id??""}><option value="">No batch</option>{batches.map((item)=><option value={item.id} key={item.id}>{item.name}</option>)}</select><button className="btn-primary">Save</button></ActionFeedbackForm>)}</div></section>
    <StudentCsvImport />
  </DashboardShell>;
}
