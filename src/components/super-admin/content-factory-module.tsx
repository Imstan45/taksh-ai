...
export default function Home() {
  ...
  const [config, setConfig] = useState({ gemini: false, supabase: false, database: false });
  ...
  async function loadCurriculum(course = "", module = "", topic = "") {
    ...
    try {
      ...
      const response = await fetch(`/api/content-factory/curriculum?${params}`);
  ...
  async function loadConfiguration() {
    try {
      const response = await fetch("/api/content-factory/config");
      if (response.ok) {
  ...
  async function loadAssets(nextSearch = search, nextStatus = statusFilter) {
    try {
      ...
      const response = await fetch(`/api/content-factory/content?${params}`);
      const data = await response.json();
  ...
  async function generate() {
    ...
    try {
      ...
      if (config.supabase) {
        const saveResponse = await fetch("/api/content-factory/content", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function runCurriculumLoop() {
    ...
    setNotice("");
    try {
      const curriculumResponse = await fetch("/api/content-factory/curriculum?all=true");
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        try {
          const generationResponse = await fetch("/api/content-factory/generate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  async function saveAsset(status?: AssetStatus) {
    ...
    try {
      ...
      const response = await fetch(`/api/content-factory/content/${selectedAsset.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          content,
    ...
    <main>
          ...
          {!result ? (
            ...
            <div className="result">
              ...
              {selectedAsset && <div className="review-actions">
                ...
                {selectedAsset.status === "published" && <a className="publish-action" href="/super-admin/courses">Assign to course or student</a>}
      ...
      {screen === "settings" && <section className="settings-page">
        ...
        <div className="setup-guide">
          ...
          <span className="step">02</span><div><h3>Configure generation</h3><p>Set <code>GEMINI_API_KEY</code> in the main Taksh deployment. The existing Taksh session protects this module.</p></div>
