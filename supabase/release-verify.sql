...
begin
  select string_agg(name,', ') into missing from unnest(array[
    ...
    'faculty_feedback','learning_activities','activity_submissions','notifications',
    'aptitude_assessment_sessions','invitations','audit_logs'
  ...
  select string_agg(item,', ') into missing from (values
    ...
    ('academic_batches.academic_year_id'),('academic_batches.semester_id'),
    ('faculty_assignments.course'),('user_academic_memberships.roll_number'),
  ...
  if exists(select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
    where n.nspname='public' and c.relname in('user_roles','departments','semesters','academic_batches','student_course_assignments',
  ...
  if not exists(select 1 from pg_trigger where tgname='memberships_institution_guard' and not tgisinternal)
    ...
    or not exists(select 1 from pg_trigger where tgname='semesters_institution_guard' and not tgisinternal)
    or not exists(select 1 from pg_trigger where tgname='academic_batches_semester_guard' and not tgisinternal)
  ...
  if not exists(select 1 from public.schema_migrations where version='202608110001')
    then raise exception 'Institutional semester migration is not recorded'; end if;
  if not exists(select 1 from public.schema_migrations where version='202608130001')
    then raise exception 'College learning workflow migration is not recorded'; end if;
end $$;
