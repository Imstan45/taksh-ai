...
describe("Sales Rep referrals",()=>{
 it("creates human-readable, non-numeric referral codes",()=>{const code=salesReferralCode("Surya Kumar");expect(code).toMatch(/^SURYAKU[A-F0-9]{4}$/);expect(code).not.toMatch(/^\d+$/)});
